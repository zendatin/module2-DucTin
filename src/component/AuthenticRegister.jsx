...
import styles from "../Scss/SignInBootstrap.module.scss";
import { NavLink } from "react-router-dom";

const USER_REGEX = /^[A-z][A-z0-9-_]{3,23}$/;
const PWD_REGEX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%]).{5,24}$/;
/* const REGISTER_URL = '/api/data'; */

const AuthenticRegister = () => {
    ...
    <>
      {success ? (
        <section>
          <h1>Success!</h1>
          <p>
            <NavLink to="/signinauthentic">Sign In</NavLink>
          </p>
        </section>
      ) : (
        <section>
          <p
            ref={errRef}
            className={cx(errMsg ? "errmsg" : "offscreen")}
            aria-live="assertive"
          >
            {errMsg}
          </p>
          <h1>Register</h1>
          <form onSubmit={handleSubmit} className={cx("formregister")}>
...
export default AuthenticRegister;
